...
Cập nhật: 2026-09-24 (+07, sau P5.4 và fix SnackBar messenger — ScaffoldMessenger đặt sai vị trí).
...
| fix_snackbar_messenger | Fix SnackBar không hiện (ScaffoldMessenger đặt sai vị trí) | 🟡 **code xong** (theo `.plan/prompt_fix_snackbar_messenger.md`) — `ScaffoldMessenger(key: _messengerKey)` trong `root_scaffold.dart` bị đặt **trong** `Scaffold.body` (dưới `Scaffold`, ngược thứ tự đúng của Flutter — xác minh bằng chính SDK: `ScaffoldMessengerState._isRoot`, dòng 211–245) ⇒ SnackBar bị vẽ trong `Scaffold` của tab **offstage** trong `IndexedStack` ⇒ **bấm ở tab Trang chủ/Cài đặt thì không thấy gì** (Test LLM im lặng — đúng triệu chứng user). **Fix:** đảo 2 widget (messenger ra NGOÀI, `Scaffold` vào trong; `appBar`/`IndexedStack`/`BottomNavigationBar` không đổi nội dung); **KHÔNG** đổi `enqueueSnack`/`_drainSnackQueue`; `HistoryScreen`/`StatsScreen` **giữ nguyên** `Scaffold` riêng (Flutter tự hiện đúng 1 SnackBar qua `_isRoot`). 4 test mới (`snackbar_visibility_test.dart`) bấm nút trong cây `RootScaffold` đầy đủ + bất biến "SnackBar không có tổ tiên `IndexedStack`"; **đã kiểm test biết đỏ** (bản cũ: 4/4 đỏ). Thêm seam tuỳ chọn `TestLlmService? testLlm`. Sửa kèm **time-bomb** ở `p5_4_catchup_test.dart` (fake DAO dùng `DateTime.now()` thật vs service bơm `now:` cố định — test chỉ xanh trước 03:00 24/09/2026; hằng `_fixedNow` dùng chung, test-only; xem A72); **428/428 pass**, analyze sạch; OCR không khả dụng + AgentMemory UNREACHABLE ⇒ fallback review mặc định. **DoD còn thiếu test tay trên máy thật** (checklist `.plan/manual_test_snackbar_messenger.md`) — gộp K51 — xem `.plan/fix_snackbar_messenger-result.md` |
