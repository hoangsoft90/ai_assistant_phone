...
- [x] Rà **5 file native còn lại** (ASR + capture + VAD, 1.332 dòng) theo đúng cách làm K45 ⇒ **1 lỗi CRASH + 2 lỗi cùng họ**, đã sửa: (1) `WhisperChunkEngine.close()` free model native trong lúc `nativeTranscribe` còn chạy (use-after-free ⇒ crash tiến trình) **và** `RejectedExecutionException` ném ra từ `finally` khi executor vừa đóng (Android giết tiến trình với exception không bắt được ở **bất kỳ** thread nào) — ca thật: **dừng nghe đúng lúc đang transcribe**; (2) `MicCaptureEngine` nhánh lỗi nhả `AudioRecord` theo **field dùng chung** ⇒ có thể nhả mất recorder MỚI sau restart; (3) `stop()` bỏ qua join thread đã tự dừng vì lỗi ⇒ `release()` đóng VAD dưới chân thread còn `analyze()`. Kèm `@Volatile` cho 4 field dùng chung giữa 2 thread. Đã rà và thấy **sạch**: `VoskStreamingEngine.release()`, `VadDetector`, `CaptureChannelBridge`.
- [ ] ⚠ Nợ **K47**: các bản sửa native vòng 2 chưa verify trên máy — dừng nghe **đúng lúc** Whisper đang transcribe (không crash + lần sau vẫn chạy), đổi engine giữa lúc đang transcribe, và lỗi mic giữa lúc thu rồi bật lại.
