...
# PROJECT
<!-- Phần RIÊNG của repo `ai_assistant_phone`. Đã điền cụ thể. Khi bản chất dự án đổi (thêm phase,
     đổi ràng buộc cứng, đổi stack) thì cập nhật ở đây **và** ở `context.md` + `.project/` cho khớp. -->

## Role & Context

`ai_assistant_phone` — **Trợ lý giao tiếp**: app Android (cá nhân, một người dùng) nghe hội thoại
bằng **mic điện thoại**, bóc băng **offline**, gợi ý câu trả lời, và **đọc gợi ý qua tai nghe
Bluetooth (chỉ A2DP)** để chỉ người dùng nghe được.

- **Tech stack:** Flutter 3.47.2 (Dart `^3.13.2`), **Android only**, `applicationId`
  `com.aiassistant.phone`, **minSdk 26**, target/compileSdk 36, NDK 28.2.13676358, Kotlin 2.4.0,
  AGP 9.1.0, Gradle 9.3.1. ASR local: PhoWhisper (chính) + Vosk (dự phòng).
- **Vai trò của agent:** dev chính. Người dùng **không đọc code** — tự kiểm Precondition, tự đối
  chiếu DoD **có bằng chứng**, báo cáo theo mục 5 của `.plan/AGENT_INSTRUCTIONS.md`, và ghi thành
  `.plan/<PHASE>-result.md`.
- **Trạng thái hiện tại:** mới có khung P0.5 (service + audio session + SQLite + màn hình chẩn đoán).
  **Chưa có tính năng sản phẩm nào. App chưa từng chạy trên máy thật.** Đừng giả định có auth /
  backend / payment — dự án này **không có** và **sẽ không có**.
- **Kiến thức kiến trúc:** đọc `.project/README.md` (entry point cho toàn bộ knowledge base).
...
Thứ tự đọc khi cần tra cứu sâu (chỉ đọc khi câu hỏi thực sự cần — tránh tốn token):

1. `.project/README.md` → điều hướng tới các file còn lại.
2. `.project/overview.md` → **mục "Ràng buộc cứng"** (6 ràng buộc không được vi phạm ở bất kỳ phase).
3. `.project/openspec.md` → tiến độ phase + bug/nợ kỹ thuật đã biết (K1–K10) + todo kế tiếp.
4. `.project/architecture.md` → quy ước đặt file theo tầng (đọc **trước khi** tạo file mới).
5. `.project/patterns.md` → pattern đang dùng + pattern **chưa có** (đừng tưởng nhầm là đã dùng).
6. `.project/modules/<module>.md` → chi tiết một mảng code trước khi sửa nó.

**Ghi chú riêng của repo này:**

- `.plan/` **bị gitignore** nhưng là nguồn spec thật (`plan_final_v2.md`, `prompt_P<phase>.md`,
  `P0-result.md`, `P0_5-result.md`). Đọc được, **không** commit, và khi trích dẫn cho người khác thì
  chép sang `.project/`.
- `spikes/p0_audio/` là **code thăm dò P0, không phải code sản phẩm** — được phép xoá cả thư mục.
  Trong đó có `TtsTest` phát âm thanh trực tiếp: **tuyệt đối không tái sử dụng** (xem Critical Rules).
- Máy dev này **không có Android SDK/NDK** (chỉ có `adb`) ⇒ **không build APK tại chỗ**; build qua
  GitHub Actions. `/home` gần đầy (~396MB) ⇒ việc nặng chạy ở `/tmp`.
- Repo hiện **0 commit** ⇒ `git diff` rỗng, `detect_changes`/impact review chưa dùng được.

## Critical Rules (Must Follow)

> Đây là **ràng buộc cứng của sản phẩm**, không phải sở thích kỹ thuật. Vi phạm = làm lại.
...
1. **Tai nghe Bluetooth chỉ để PHÁT (A2DP một chiều).** Tuyệt đối **không** dùng
   ...
   thống sang chế độ đàm thoại (HFP/SCO) và hạ chất lượng audio.
...
3. **Không được để tiếng TTS lọt ra loa ngoài — trong bất kỳ tình huống nào**, kể cả khi tai nghe bị
   rút đột ngột giữa lúc đang phát. Từ P1F, **mọi** phát âm thanh phải đi qua `SafeTtsOutput`.
4. **ASR phải chạy offline trong app. Audio hội thoại KHÔNG được gửi lên cloud.** Chỉ `text` đã bóc
   băng mới được gửi tới LLM (P2). `INTERNET` trong Manifest **chỉ** cho LLM.
5. **Half-duplex:** đang phát TTS thì không thu, đang thu thì không phát.
6. **App cá nhân một người dùng:** không thêm auth / tài khoản / payment / backend / multi-tenant.
7. **Không bao giờ xoá hoặc tạo lại DB của người dùng** để tiện cho việc sửa schema — transcript là
   dữ liệu nhạy cảm (nội dung hội thoại thật). Mọi thay đổi schema phải qua migration.
8. **Không lưu secret/API key vào SQLite, file cấu hình hay log.** Chỉ `SecureStore` (keystore OS).
9. **Không thêm logic audio vào `onRepeatEvent` của foreground service khi chưa tới P1A**, và không
   đổi thứ tự khởi tạo trong `main()` (service → audio session → DB → `runApp`).
10. **`listeningTaskCallback` phải là hàm top-level + `@pragma('vm:entry-point')`** — lỗi này
    **không lộ ra khi test**, chỉ lộ khi app chạy nền trên máy thật.
11. **Tên OpenSpec change/spec không được bắt đầu bằng chữ số** (xem `.project/overview.md` mục 6).
12. **Sửa `lib/audio/app_audio_session.dart` hoặc bất cứ thứ gì liên quan phát/thu âm là vùng loại
    trừ Ponytail** ⇒ phải dừng lại hỏi người dùng trước khi commit.
...
- **Trước khi bắt đầu một phase:** đọc `.plan/prompt_<PHASE>.md` + `.plan/AGENT_INSTRUCTIONS.md`.
  Tự kiểm **Precondition có bằng chứng** (lệnh thật, không suy đoán). Nếu Precondition không đạt:
  **dừng và hỏi người dùng**, không tự chọn hướng — trừ khi người dùng nói rõ "waive precondition"
  (khi đó ghi rõ việc waive + rủi ro vào báo cáo phase).
- **Bug fixing:** chưa có bug runtime nào (app chưa chạy trên máy). Khi có, thứ tự tra:
  1. `adb logcat` lọc theo tag của `AppLogger` (`<Tag>/<level>`, ví dụ `ListeningTaskHandler/info`).
  2. `.project/openspec.md` mục 3 — danh sách nợ kỹ thuật đã biết (K1–K10); nhiều khả năng vấn đề
     đã được ghi sẵn ở đó rồi.
  3. `.project/modules/<module>.md` mục "Cảnh báo khi sửa".
  4. `LESSONS_LEARNED.md` — lỗi đã từng mắc trong dự án này, tránh lặp lại.
- **Feature add:**
  1. Xác định tính năng thuộc **phase nào** (`next.md`); nếu chưa có phase → hỏi người dùng trước.
  2. Dùng **Dart** (UI/logic) hay **Kotlin** (audio, service, native)? Logic audio thường phải ở
     Kotlin — **không** cố làm bằng Dart plugin có sẵn.
  3. Đọc `.project/architecture.md` mục 1 để đặt file đúng tầng; bọc plugin qua lớp trong
     `services/`/`audio/` (UI không gọi plugin trực tiếp).
  4. Nếu tính năng chạm vùng an toàn (audio routing, TTS, lưu trữ dữ liệu người dùng) → dừng hỏi.
  5. Cập nhật test (`flutter test` chỉ nhận file `*_test.dart`; plugin mới phải thêm stub
     MethodChannel) rồi chạy `flutter analyze` + `flutter test`.
  6. Cập nhật `.project/modules/<module>.md` (nếu module đã có file) hoặc tạo file mới khi feature
     đầu tiên của nó xuất hiện.
  7. Cập nhật `working.md` + `checklist.md`; ghi báo cáo `.plan/<PHASE>-result.md` nếu là task lớn.
- **Sau khi xong một phase:** cập nhật `.project/openspec.md` (bảng phase + nợ kỹ thuật) và `next.md`.
  **Không tự commit** — chờ người dùng xác nhận.
...
- **Branch/Commit convention:** chưa chốt (người dùng chưa quyết) — repo hiện **0 commit**, nhánh
  hiện tại là `master`. Trước khi commit lần đầu phải hỏi người dùng: có tách nhánh theo phase
  không, và chia thành mấy commit. Đề xuất mặc định (chờ xác nhận): một nhánh cho mỗi phase
  (`p0-5-bootstrap`), commit theo bước, dùng prefix `feat:`/`fix:`/`chore:`/`docs:`.
