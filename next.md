...
- **Phase hiện tại: sau P7 — các phase bổ sung P2.1 (custom LLM provider) + P5.1 (Lịch sử + retention) + P5.2 (tên phiên) + P5.3 (nav 4 tab) + issue1_fix/follow-up (LLM config + vòng đời phiên) + P5.4 (timeout theo use-case + phân tích bù) + **fix_snackbar_messenger (SnackBar không hiện)** code xong + review xong (**428/428 test**, analyze sạch). **fix_snackbar_messenger CHƯA commit** (chờ xác nhận user); các phase trước **đã push tới `a36ce8b`** (+docs tới `b4904d3`). **Tài liệu kết quả đầy đủ: `.plan/P2_1-result.md`, `.plan/P5_1-result.md`, `.plan/P5_2-result.md`, `.plan/P5_3-result.md`, `.plan/P5_4-result.md`, `.plan/issue1_fix-result.md`, `.plan/fix_snackbar_messenger-result.md`.** Việc còn lại của dự án là vòng test máy thật gộp (**K51**, gồm cả checklist `.plan/manual_test_snackbar_messenger.md`) với APK mới.**
...
| 13h | **fix_snackbar_messenger** SnackBar không hiện (ScaffoldMessenger đặt sai vị trí) | 🟡 code xong — 428/428 test | `ScaffoldMessenger` ra NGOÀI `Scaffold` trong `root_scaffold.dart` (trước messenger nằm trong `body` ⇒ SnackBar vẽ vào `Scaffold` của tab offstage trong `IndexedStack` ⇒ tab Trang chủ/Cài đặt không thấy gì); KHÔNG đổi `enqueueSnack`; 4 test mới bấm nút trong cây đầy đủ (đã kiểm biết đỏ); seam `testLlm`; sửa time-bomb `p5_4_catchup_test.dart` (A72). Xem `.plan/fix_snackbar_messenger-result.md` |
