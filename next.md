...
Cập nhật: 2026-09-23 chiều (+07). Nguồn: `.plan/production_roadmap.md`, `.plan/P4-result.md`, `.plan/P5-result.md`, `.plan/P7-result.md`, `.plan/P2_1-result.md`, `.plan/P5_1-result.md`, `.plan/P5_2-result.md`.

## Đang ở đâu

- **Phase hiện tại: sau P7 — các phase bổ sung P2.1 (custom LLM provider) + P5.1 (Lịch sử + retention) + P5.2 (tên phiên) + P5.3 (nav 4 tab + nút nổi toàn cục) code xong + review xong (373/373 test, analyze sạch), đang tách commit + push để CI build APK mới. Việc còn lại của dự án là vòng test máy thật gộp (K51) với APK mới.**
...
| 13e | **P5.3** Nav 4 tab + nút nổi toàn cục | 🟡 code xong — 372/372 test | `RootScaffold` (IndexedStack) + `GlobalFloatingControls` + `SessionCoordinator`; `home_screen.dart` thành shim; `floating_button.dart` 0 dòng bị đụng |
...
0. **Tách commit P2.1/P5.1/P5.2/P5.3 + docs và push (user ra lệnh 2026-09-23)** → CI build APK mới chứa:
   custom LLM provider, Lịch sử phiên, tên phiên, nav 4 tab. Sau khi CI xanh: **tải APK → cài máy**.
